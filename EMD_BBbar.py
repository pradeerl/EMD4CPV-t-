...
x_plusalt  = np.linspace((2*mpi)**2/M**2, (M-mpi)**2/M**2, 34)
s2_minalt = min(s_minus_min(x_plusalt*M**2))/M**2
s2_maxalt = max(s_minus_max(x_plusalt*M**2))/M**2

x_minusalt = np.linspace(s2_minalt, s2_maxalt, 34)
...
def weak_num(i):
    weak_og = np.array([0, i*np.pi/100, 0])
...
sample_size = 594
idx_geom = rng.choice(len(coords), size=sample_size, replace=False)
coords_red = coords[idx_geom]

loss_mat_red = ot.dist(coords_red, coords_red)
loss_mat_red /= loss_mat_red.max()

def compute_emd_for_coupling(i, idx_geom, loss_mat_red):
    ...
    p1 = ((1 + graph_c0 * np.cos(r * x1)
             - graph_s0 * np.sin(r * x1))
          * np.exp(-s * x1)).ravel()[~mask2]

    p2 = ((1 - graph_c0 * np.cos(r * x2)
             + graph_s0 * np.sin(r * x2))
          * np.exp(-s * x2)).ravel()[~mask2]
    ...
    x1_m_red = x1_m[idx_geom]
    x2_m_red = x2_m[idx_geom]
    x3_m_red = x3_m[idx_geom]
...
if __name__ == "__main__":
    ...
    results = Parallel(n_jobs=-1, backend="loky")(
        delayed(compute_emd_for_coupling)(i, idx_geom, loss_mat_red)
        for i in tqdm(phase_scan, desc="Coupling value")
    )
    results = np.array(results)
    emds_null = np.array(results[:,0,:])
    emds_phase = np.array(results[:,1,:])
    np.save("list of emds_true_g2_test.npy", emds_null)
    np.save("list of emds_phase_true_g2_test.npy", emds_phase)
